...
`/session` 接口用来 创建`create`, 销毁`destroy`, 和 查询`query` 会话`sessions` .

## 创建 Session

会话必须和一个节点相关连，并且可能需要跟任意数量的 检查(checks) 关联。

| Method | Path                         | Produces                   |
| :----------------: | :-----------------: | :---------------: |
| `PUT`  | `/session/create`            | `application/json`         |

下表展示了该API提供的支持`Blocking Queries`、`Consistency Modes`、`ACLs`
...
- `dc` `(string: "")` - 指定要操作的数据中心。 
This will default to the datacenter of the agent being queried. This is specified as part of the URL as a query parameter. 
